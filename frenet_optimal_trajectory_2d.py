...
def calc_frenet_paths(c_speed, c_d, c_d_d, c_d_dd, s0, K_FUEL):
    frenet_paths = []
    ...
    for di in np.arange(-MAX_ROAD_WIDTH, MAX_ROAD_WIDTH, D_ROAD_W):
        ...
        for Ti in np.arange(MIN_T, MAX_T, DT):
            ...
            for tv in np.arange(TARGET_SPEED - D_T_S * N_S_SAMPLE,
                ...
                tfp.cfuel = K_F * Fs
                tfp.cf = K_LAT * tfp.cd + K_LON * tfp.cv + K_FUEL * tfp.cfuel
...
def frenet_optimal_planning(csp, s0, c_speed, c_d, c_d_d, c_d_dd, ob, K_FUEL):
    fplist = calc_frenet_paths(c_speed, c_d, c_d_d, c_d_dd, s0, K_FUEL)
...
def simulation(wx, wy, ob, k_fuel, prev_paths_x = None, prev_paths_y = None):
    # initial state
    ...
    for i in range(SIM_LOOP):
        path = frenet_optimal_planning(
            csp, s0, c_speed, c_d, c_d_d, c_d_dd, ob, K_FUEL=k_fuel)
        ...
        if show_animation:  # pragma: no cover
            ...
            plt.plot(paths_x, paths_y, "vc")
            if prev_paths_x is not None:
                plt.plot(prev_paths_x, prev_paths_y, "*b")
...
def main():
    ...
    wy = [0.0, 6.0, 5.0, 6.5, 0.0, 2.5, 15.5, 10]
    # obstacle lists
    ...
    paths_1_x, paths_1_y = simulation(wx=wx, wy=wy, ob=ob, k_fuel = 0.0)

    paths_2_x, paths_2_y = simulation(wx=wx, wy=wy, ob = ob, k_fuel = 3.0, prev_paths_x = paths_1_x, prev_paths_y = paths_1_y)
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.plot(ob[:, 0], ob[:, 1], "xk")
        #plt.plot(path.x[1:], path.y[1:], "-or")
        plt.plot(paths_1_x, paths_1_y, "vc")
        plt.plot(paths_2_x, paths_2_y, "*b")
        plt.xlim(0, 100)
        plt.ylim(-50, 50)
        plt.title("v[km/h]:" + str(c_speed * 3.6)[0:4] +
                  "    a[km/h2]:" + str(c_accel * 3.6)[0:4] +
